...
class GeminiSentiment(APIView):
    ...
    def post(self,request):
        try:
            ...
            prompt = self.prompttype(user_review, history)
            rawresponse=response.getresponse(prompt)
            print(rawresponse)
            cleaned = rawresponse.replace("```json", "").replace("```", "").strip()
            classification = json.loads(cleaned)

            history.append({"userReview": user_review, "response": classification})
...
class Prompt(GeminiSentiment):
    prompttype = Prompts().prompt
